...
from pytorch_GAT.utils.constants import LayerType
...
# LPGAT definition
class LPGAT(torch.nn.Module):
    def __init__(self, in_feats, mid_feats, emb_feats, out_feats):
        super().__init__()
        self.gat_1 = GAT(
            num_of_layers=2,
            num_heads_per_layer=[4, 1],
            num_features_per_layer=[in_feats, mid_feats, emb_feats],
        ...
        self.gat_2 = GAT(
            num_of_layers=2,
            num_heads_per_layer=[4, 1],
            num_features_per_layer=[emb_feats, mid_feats, out_feats],
            add_skip_connection=True,
            bias=True,
            dropout=0.1,
            log_attention_weights=False
        )
        self.edge_pred = torch.nn.Linear(out_feats, out_feats, bias=False)

    def encode_1(self, x, edge_index):
        z, _ = self.gat_1((x, edge_index))
        return z

    def encode_2(self, z, edge_index):
        z, _ = self.gat_2((z, edge_index))
        return z

    def decode(self, z, edge_label_index):
        # return self.edge_pred(torch.hstack([z[edge_label_index[0]], z[edge_label_index[1]]]))
        a = z[edge_label_index[0]]
        b = z[edge_label_index[1]]
        mid = self.edge_pred(a)
        return (mid * b).sum(-1)